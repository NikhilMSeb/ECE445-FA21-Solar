...
**Outcome:** Our TA Evan was able to spend some time today helping us fix some of our problems. One of these issues was the aforementioned ESP32 stand-alone inability to run. But Evan was able to show us that the ESP32 needs to have its enable pin settings such that it can flash and reload the program when being independently powered in order to run. It turned out that when designing the PCB my teammates had tied the ESP32's enable pin to low, which is what was causing our issue. On leaving this pin floating and connecting the programmed ESP32, followed by a power boot, the ESP32 was able to execute as programmed independently! \
We also spent more time today on sorting out the ADC, and this time with Evan's supervison. We changed our testing approach to see if we were able to manually read based on address from the ADC through ESP32 analog input. While we were able to read through the I2C bus with our earlier wiring itself, these readings were just default/zero values. We then tried to write to the ADC through analog in an effort to specify it's address location, and also trying to reconfigure it to different modes, but none of these approaches worked. What we realized was happening was that the ADC was not sending back an ACK acknowledgement bit to the ESP32, without which the master-slave connection on the I2C bus will not be established. Because we could not find any logical source of this anomaly, our TA told us to adjust our design to remove this isolating component and directly read our voltage measurements from our voltage division circuitry. \
Finally, we also followed up with the Machine Shop as planned. After speaking with them, they were able to drill precise holes in all our PCB boards (including test ones) as needed so that when completed we can directly mount the same into our Attaboxes for final implementation. 

## 11/30/2021: Continuing Firmware Work and Hardware Integration (4)
**Objectives:** Continuing to work on testing and firmware integration for available parts, along with PCB testing as a whole

**Outcome:** The first thing that we worked on today was wiring and programming to read from our ACS712 Current Sensor. We received these sensors attached to a dev-kit board, and we decided we would use this for testing and then de-solder the sensor chip itself before attaching it to our PCB board. Following its wiring, I was able to program the ESP32 to be able to read from the current sensor directly through an ADC GPIO pin (4) - the ESP32 would read a raw value, which would then be converted to a voltage by using the supply voltage and a reference voltage. This initial approach would be the same even for the voltage divider measurement, so I took some time to understand the approach and also figure it out exactly as it was not perfect the first time. \
Then, an offset voltage (when sensor is powered but measuring 0A) is subtracted from the voltage we calculated before, and the current sensitivity of the sensor is used to calculate what current that voltage value corresponds to. While I got this to work, we realized that the accuracy of our measurements were off by about 10% - much higher than the 1.5% that was our requirement! To fix this issue I calibrated all the sensor output values in our acceptable input range (up to 6A), and then we were able to bring accuracy to within 1%. The plot for calibration is the first picture seen below along with the equation we found, and the 2 following pictures show the ESP32's outputs when measuring 2A and 4A respectively: 
...
Throughout this time, we have been updating our PCB board and testing for correct integration. I have constantly updated an independent Arduino sketch that is the integrated version of each of the individual programming and tests that I have done. We have also been adding pull-up resistors as and when needed as per what we were finding during modular testing as well. So far all integration tests with the ESP32 on the PCB have been successful on the hardware and firmware sides! 
...
**Outcome:** The first thing to complete today was to analyze the remaining current sensors that we had in hand to see if calibration and sensitivity would change enough for me to need to separately program each case. We realized this was not the case and that the same programming from yesterday would continue to work for all the sensors. We then de-soldered and added just the current sensor chips to our PCB as planned and tested if it worked with the board onto the OLED display, and this was successful! This met 1 requirement and verification of our monitoring subsystem with respect to current monitoring. \
We then moved onto our voltage measurement, where there was no further circuitry changes that were needed as our PCB had soldered on resistors in place. I used a similar appraoch to current measurement using a different ESP32 ADC GPIO pin (12), except here after obtaining the raw voltage I needed to reverse engineer the input voltage value from the measured stepped-down value based on the voltage division circuitry. While I was able to do this successfully, we once again saw that the accuracy for the values being read were about (or in some cases more than) 10%. Once again, I calibrated the values within our possible input range - 0V to 25V - and could then bring our accuracy to within 1%! The plot for calibration is the first picture seen below along with the equation we found, and the 2 following pictures show the readings on the PCB for our voltage measurements. Note that the OLED display in the pictures show "Meas. Vol." (calibrated output of the voltage divider), "Real Vol." (voltage that is scaled up but no calibrated), and "Adj. Vol." (final calibrated voltage):
...
Also note that since the voltage divider circuitry is directly on the PCB, all of the testing for this measurement was done with full PCB integration already through OLED display outputs. So this fully meets another requirement and verification of our monitoring subsystem with respect to voltage monitoring, and we had it ready to move forward. Now that we had completed progamming and basic integration testing of all the 3 sensing components (temperature, current, voltage), we now needed to check if we could get it all integrated successfully with the ESP32 stand-alone so that it will all be displayed on the OLED on the PCB. I updated my independent sketch file to have all of this code in one upload, and then we were able to test the same. The 2 pictures below show the successful results of these integration tests! Note that since we did not have the configuration/darlington array circuitry functional at this time, we could not test voltage and current at the same time since they shared an input terminal. But we just separately probed 10V and 4A into the input with the rest of the ciruict staying the same and got the below outcomes (with temperature measurement constantly functioning). This proves successful functionality, and at this point we have met almost all of our monitoring subsystem requirements and 1 of our 2 large microcontroller subsystem requirements!

![unnamed](https://user-images.githubusercontent.com/46250395/145523997-ee7c57c9-0d00-424b-82cf-6ef549c3e122.jpg) \
![unnamed](https://user-images.githubusercontent.com/46250395/145524056-9f4567ed-4dde-4c89-86c3-ba3d1d24d48a.png)
...
**Outcome:** With most of my firmware objectives now met, I needed to tie the loose ends and integrate (along with testing) the remainder of my software functionalities and the Research Hub. The first of these was to finish the implementation of my ManyToOne relationship approach to model management on Django to allow for data storage as discussed earlier. I was able to directly implement this with 3 new models for the 3 measurements, and each with a ForeignKey that linked it to a dedicated solar panel model by ID. The picture below shows the terminal output when linking multiple models of measurement to independent solar panel models - we can see the successful management of these and the terminal outputs to prove it: 
...
The next update needed was to integrate this ManyToOne appraoch with the fully functional forward path of software (successful POST from ESP32 to Django project which is stored and updated correctly). For this I needed to remove the earlier serializer class, url path, and view configuration that I had in place for the API that took POST requests for direct panel model updates. Instead I made 3 dedicated serializer classes, url paths, and view configurations to handle each POST request to the voltage, current and temperature measurement models. After streamlining my Django back-end to reflect this completely, I was able to test the same entirely from the ESP32 all the way to the front-end update. The picture below shows the successful periodic ESP32 POST requests to the Django project on the serial monitor, and the server terminal response shows the three measurement types being periodically received and stored without any problems! I also had a display on the front-end to show the last 20 values (required filtering) received for each measurement for each panel and this correctly proved the success of this requirement of the Research Hub subsystem and its functional verification. 
...
Now that I had the back-end of Django working to successfully update the right models based on wireless input, I needed to adapt my simple value prints for each panel into the data visualizations that have been planned and which are a requirement. To achieve this, I used the *plotly* Python library (for the implementation of which I had already completed the setup earlier) to simply take the immediate last 20 values received for each measurement for each panel that I was already filtering at the front-end, and display the same as an interactive and aesthetic Plotly plot. The 3 pictures below show the corresponding voltage, current and temperature plots seen on panel-specific pages after successful implementation! This also met another requirement of the Research Hub subsystem and shows the success of its functional testing: 
...
Finally, now that the forward movement of software (observational data flow) was good to go, I needed to complete the backward flow that would be used to update the interface box of a new user-input configuration for monitoring. For this I needed to add GUI for user to actually input something, and then program the back-end to take this input via a POST request and update the ESP32 of the same. Since I already had the ESP32 successfully proven to be able to receive, I added a view to the "panels" application that (for testing was periodic and cyclical) sent the 3 possible panel configurations to the ESP32. The picture below shows the successful reception of these test configurations (just raw text for testing) at the ESP32's end when it runs the asynchronous web server code that I wrote for it to receive POST requests and store them: 
...
After all this, I wanted to ensure that the software side of the project that was working fine independently and the firmware and hardware that were working together perfectly would combine to form our total integrated solution as expected. Unfortunately, for whatever reason, when powered by an independent power supply with the WiFi-based code uploaded, the ESP32 was not able to execute any of the programmed code by itself. I started investigating this but could not make too much headway today because I needed to focus on the last functionality of the Research Hub, and I did not have my teammates' helping at the time as they had run into some hardware issues. 
...
**Outcome:** The only requirement of the software side of the project and the Research Hub subsystem left was to link the wireless communication abilities that I already have functional to an implemented GUI for the user to be able to send input configurations to the ESP32 and the interface box. As mentioned, since I had the back-end itself functional, I just needed to add some GUI for a form to be able to take in user input and then have this trigger the right view to then trigger the POST request functionality. I was able to add a simple form to the end of each panel-specific page that took in an input configuration choice of 1, 2 or 3, and then triggered a corresponding POST view on the back-end. The 2 pictures below show the successful functioning of this system - the first picture is simply the GUI itself, and the second picture is the forced HTTP response I displayed to show that the Django back-end was able to receive and process the user input to trigger the correct view from *views.py*, which in turn triggers the working POST request to the ESP32. At this point, all the functional requirements of the Research Hub subsystem and the software side of the project have been met and functionally verified: 
...
After this, my team and I met in the lab in the morning for one final run through of anything to be adjusted and tested before our demo. In this time I was able to successfully execute all software functionality over a new network (a hotspot mimicking a router), and my teammates were able to fix the issue they were having with the physical conections of the darlington array and relay setup. Our TA Evan helped find out that the relays themselves were actually soldered on in reverse, which is what was causing the issues, not anything wrong with the logical and programmed working I had completed and tested. \
Following this we completed our Final Demo where we could showcase the successful functioning of all 4 of our subsystems, and also the proof our integrated working of 3 of our subsystems. While I had tried to explore why the ESP32 couldn't handle managing the WiFi-based functionality when independently powered (possible reasons are cheaply-made/faulty WiFi modules that can't handle the bandwidth without micro-USB tethering, or potential issues in the wiring of ESP32 pins on the PCB), I could not get it to work in time. In the end this was the only large piece of the project that did not allow for a fully integrated and functional solution. 
